...
-- @TAGLIST_BUTTON@
bindings.taglist_buttons = gears.table.join(
	awful.button({ }, 1,
	function(t) 
		t:view_only() 
	end),
...
bindings.keyboard = gears.table.join(
	...
	function()
		...
		{description = "swap with next cli by index", group = "client"}),

	awful.key({ modkey, "Shift"   }, "k", 
	function() 
		awful.client.swap.byidx( -1)
	end,
		{description = "swap with prev cli by index", group = "client"}),
	...
	function()
		...
		{description = "system volume up", group = "awesome"}),

	awful.key({modkey},		"Down",
	function()
		os.execute(string.format("amixer -q set %s 1%%-", "Master"))
	end,
		{description = "system volume down", group = "awesome"})
